...
namespace lux {
	...
	// lights and put them in a lux format
	MStatus	Export::doIt( const MArgList& args) {
		MStatus status;
		MDagPath tempDagPath;
		
		if (args.length() < 5) {
			MGlobal::displayError("At least 5 arguments are required for LUXExport");
		...
		fout << setprecision(9) << setiosflags(ios_base::fixed);	// make sure we keep the output readable to lux
