 ...
 *
 *	Edited by Dom COCO on 04/12/2007
 */
...
void* LUXView::creator() {
	return new LUXView;
}

MStatus LUXView::doIt(const MArgList &args) {
	Tga tgaFile;
	MStatus status;
	
	if (args.length() < 1) {
		MGlobal::displayError("LUXView must have a valid targa file to launch");
		return MStatus::kFailure;
	}

	if (!MRenderView::doesRenderEditorExist())
	{
		setResult( "Cannot LUXView in batch render mode. "
	...
	{
		MGlobal::displayError( "LUXView: error occured in startRender" );
		free(tgaFile.data);
		return MS::kFailure;
	}
	...
	{
		MGlobal::displayError( "LUXView: error occured in updatePixels." );
		free(tgaFile.data);
		return MS::kFailure;
	}
	...
	{
		MGlobal::displayError( "LUXView: error occured in refresh." );
		return MS::kFailure;
	}
	
	if (MRenderView::endRender() != MS::kSuccess)
	{
		MGlobal::displayError( "LUXView: error occured in endRender." );
		return MS::kFailure;
	}
