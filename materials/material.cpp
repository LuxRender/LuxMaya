...
namespace lux {
	...
	Material* Material::MaterialFactory(MObject &set) {
		...
		if (depNode.typeName() == "blinn") {
			return new Substrate(depNode);
		} else if (depNode.typeName() == "lambert") {
			return new Matte(depNode);
		} else if (depNode.typeName() == "anisotropic") {
			return new Dummy(depNode);
		} else if (depNode.typeName() == "surfaceShader") {
			return new AreaLight(depNode);
		} else if (depNode.typeName() == "phong") {
			return new Glass(depNode);
		} else {
			return new Matte(depNode);
		}
