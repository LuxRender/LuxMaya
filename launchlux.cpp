...
void* LaunchLUX::creator() {
	return new LaunchLUX;
}

MStatus LaunchLUX::doIt( const MArgList &args ) {
	MString arg;
	MStatus status;
	
	if (args.length() < 1) {
		MGlobal::displayError("LaunchLUX must have an input file to launch");
	...
	// get the Lux search path from the environment
	string searchpath = getenv("LUX_SEARCHPATH");	// Changed to Luxrender path
	if (searchpath == "LUX_SEARCHPATH=") {			// Changed to Luxrender path
		MGlobal::displayError("LUX_SEARCH path not set"); 
		return MStatus::kFailure;
	}
	
	// set executable path
	string executable = searchpath + "\\lux \"";
...
DWORD WINAPI progress(void *arg) {
	...
	MProgressWindow::setTitle("LUX Rendering");
